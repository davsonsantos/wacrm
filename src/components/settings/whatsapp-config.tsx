...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    if (!config && (!accessToken.trim() || !tokenEdited)) {
      toast.error('Access Token é obrigatório para a configuração inicial');
      return;
    }
      ...
      if (data.registered === false && data.registration_error) {
        ...
        toast.success(
          data.phone_info?.verified_name
            ? `No ar — ${data.phone_info.verified_name} agora pode receber eventos.`
  ...
  async function handleReset() {
    if (!confirm('Isso excluirá a configuração atual do WhatsApp para que você possa digitá-la novamente. Continuar?')) {
      return;
    }
    ...
    try {
      ...
      toast.success('Configuração limpa. Agora você pode digitar suas credenciais novamente.');
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
      setAccessToken('');
