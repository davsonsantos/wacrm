...
export function MessageComposer({
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === "ai_not_configured") {
          ...
          toast.error(data.error ?? "Não foi possível redigir uma resposta.");
        }
        return;
      }
      ...
      toast.error("Não foi possível contatar o assistente de IA.");
    } finally {
      setDrafting(false);
    }
  ...
  const finalizeRecording = useCallback(
    async (bytes: Uint8Array) => {
      ...
      if (file.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
        toast.error("A gravação é longa demais (acima de 16 MB).");
        return;
      }
      setBusy(true);
      try {
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error("Gravação de voz não é compatível com este navegador.");
      return;
    }
    try {
